...
void main(void) {
  ...
  vec4 sample = texture2D(uShadowTexture, vTexCoords);
  ...
  //vec4 lightSpaceNDC = vec4(vWorldPosition, uLightVPMatrix);
  //vec4 lightSpaceNDC = vec4(((vWorldPosition.x + 1.0)/2.0), ((vWorldPosition.y + 1.0)/2.0), ((vWorldPosition.z + 1.0)/2.0), 0);
  ...
  //texture(lightSpaceUV, vTexCoords)
