...
void main(void) {
    ...
    // texture2D(uTexture, vTexels, uAlpha).rgb;
    ...
    // commented out due to B req #8.
    //vec3 finalColor = ambient + diffuseColor + specularColor; // ambient + diffuseColor + specularColor;
    
    // "use only the diffuse contribution (no ambient or specular)." from todo 8
    vec3 finalColor = diffuseColor; // ambient + diffuseColor + specularColor;
    ...
    gl_FragColor = vec4(lambert * finalColor, albedo[2]);
    // gl_FragColor = vec4(t2d, uAlpha);
}
